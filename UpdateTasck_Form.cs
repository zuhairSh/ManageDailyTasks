...
namespace MyProject
{
    public partial class UpdateTasck_Form : Form
    {
        public UpdateTasck_Form()
        {
            InitializeComponent();
        }
        ...
        void SaveTask()
        {
            ...
            // ... (اترك الجزء العلوي من الدالة الخاص بـ MyList كما هو تماماً دون تغيير) ...

            // هذا هو الجزء الوحيد الذي سنعدله في الأسفل:
            TaskManager_Form mainForm = (TaskManager_Form)this.Owner;

            if (mainForm != null && mainForm.listView1.SelectedItems.Count > 0)
            {
                // الحصول على رقم السطر المحدد حالياً في الشاشة الرئيسية
                int selectedIndex = mainForm.listView1.SelectedIndices[0];

                // استبدال السطر القديم بالسطر الجديد المعدل وخلاص!
                mainForm.listView1.Items[selectedIndex] = MyList;
            }
